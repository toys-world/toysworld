...
contract ToysCRM  is ERC721Recipient{
    ...
    uint256 private one_gems = 1000000000000000000;
    ...
    function paySnap(uint256 _transId,uint256 _amount) external {
        ...
        snapMapping[_transId]._addr = msg.sender;
    ...
    function buyGems(uint256 _amount) external {
        ...
        uint256[] memory getAmountOut = _router.getAmountsOut(one_gems,path);

        uint256 one_usdt_out = getAmountOut[1];

        uint256 debit = _amount.mul(one_usdt_out).div(one_gems);

        token.safeTransferFrom(msg.sender,address(this),debit);
        token.safeTransfer(company_address,debit);

        IERC20(gemsAddress).transfer(msg.sender,_amount);
    ...
    function buyWipes(uint256 _amount) external {
        ...
        require(token.allowance(msg.sender,address(this)) >= debit,"Approval");
        
        token.safeTransferFrom(msg.sender,address(this),debit);
        token.burnToken(debit);
    ...
    function addGemsPayout(address[] calldata _addr , uint256[] calldata _payout) external onlyAllowed {
        uint8 i=0;
        
        for(i;i<_addr.length;i++) {
            users[_addr[i]].totalGemsPayout = users[_addr[i]].totalGemsPayout.add(_payout[i]);
            users[_addr[i]].balanceGems = users[_addr[i]].balanceGems.add(_payout[i]);
        }
    }

    function claimGemasPayout(uint256 _amount) external {

        require(users[msg.sender].balanceGems >= _amount,"Not Enough GEMS");
        
        IERC20 token = IERC20(gemsAddress);
        
        users[msg.sender].balanceGems = users[msg.sender].balanceGems.sub(_amount);
        token.safeTransfer(msg.sender,_amount);
    }
    ...
    function withdraw() public onlyOwner {
        ...
        address payable _temp = payable(company_address);
